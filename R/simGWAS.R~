...
##' @author Mary Fortune
expected_z_score<-funtion(N0,N1,snps,W,gamma.CV,freq){
...
##' @author Mary Fortune
simulated_z_score<-funtion(N0,N1,snps,W,gamma.CV,freq,df_control){
	GenoProbList<-make_GenoProbList(snps,W,freq)
	gamma.sim<-c(compute_gamma0(N0,N1,W,gamma.CV,freq),gamma.CV)
	exp_z_score<-est_statistic(N0,N1,snps,W,gamma.sim,freq,GenoProbList)
	...
	sim_z_score<-c(rmvnorm(n=1,mean=est_z_score,sigma=LD))
}
